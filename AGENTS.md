...
**ExtendDB** is an open source DynamoDB-compatible runtime backed by TiDB storage, released under the Apache 2.0
...
The `TableEngine` trait in `crates/storage/src/lib.rs` defines the storage interface. The concrete implementation is `storage-tidb` (TiDB).
...
4. Implement the storage behavior in `crates/storage-tidb/src/` when the operation requires persistence changes
...
- **Database:** `sqlx` for TiDB/MySQL protocol access
...
- Crates: `extenddb-<component>` (e.g., `extenddb-core`, `extenddb-storage-tidb`)
- Modules: snake_case
- Types: PascalCase
...
4. **TiDB must be running** — builds and tests require a reachable TiDB endpoint.
