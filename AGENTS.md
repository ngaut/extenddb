...
**ExtendDB** is an open source DynamoDB-compatible runtime backed by pluggable PostgreSQL or TiDB storage, released under the Apache 2.0
...
- **Storage backends:** TiDB 8.5.4+ by default; PostgreSQL 14+ when explicitly built with the `postgres` feature/backend
...
- **Default:** `storage-tidb` (TiDB)
- **Optional:** `storage-postgres` (PostgreSQL, enabled explicitly with the `postgres` feature)
...
extenddb (bin)
  └─> extenddb-server
       ...
       ├─> extenddb-auth
       ├─> extenddb-storage-tidb (default feature)
       └─> extenddb-storage-postgres (postgres feature)
```
...
- TiDB 8.5.4+ for the default backend, or PostgreSQL 14+ only when explicitly using the `postgres` feature/backend
...
4. **The selected storage backend must be running** — default builds require a reachable TiDB endpoint; explicit PostgreSQL builds require PostgreSQL.
...
- Prefer TiDB-native capabilities over local coordination code: online DDL for schema transitions, timestamp snapshot reads for point-in-time reads, native TTL for item expiration, native secondary indexes for GSIs/LSIs, follower reads for eventually consistent reads, Resource Control/resource groups for distributed capacity governance, native row-id sharding/pre-split Regions plus `merge_option=deny` table attributes for append-heavy internal/shared/user data tables, and BR for backup/restore.
