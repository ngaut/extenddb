...
//! Account management operations for `TidbCatalogStore`.
...
impl TidbCatalogStore {
    pub(crate) async fn create_account_impl(
        ...
        let result = sqlx::query("INSERT INTO accounts (account_id, account_name) VALUES (?, ?)")
            .bind(account_id)
            .bind(account_name)
            .execute(self.pool())
            .await;
    ...
    pub(crate) async fn delete_account_impl(&self, account_id: &str) -> OpResult<()> {
        ...
        let has_tables: bool =
            sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM tables WHERE account_id = ?)")
                .bind(account_id)
                .fetch_one(&mut *tx)
                .await
                .map_err(|e| {
